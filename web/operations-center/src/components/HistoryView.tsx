...
export function HistoryView({ context, selection, onObservation, onProposal }: { context: AppContext; selection: WorkloadSelection | null; onObservation: (id: string) => void; onProposal: (name: string) => void }) {
  ...
  const timestampedEvents = events.filter(event => event.temporalClass === "TIMESTAMPED");
  const untimestampedFacts = events.filter(event => event.temporalClass !== "TIMESTAMPED");
  ...
  const renderEventList = (entries: HistoryEvent[], label: string) => <section className="history-event-group"><h3>{label}</h3>{!entries.length ? <p className="muted">No entries in this group match the current filters.</p> : <div className="timeline">{entries.map(event => <button type="button" className={`timeline-event ${refKey(event) === selectedKey ? "selected" : ""}`} data-testid="history-event" key={`${refKey(event)}-${event.detailCode}`} onClick={() => setSelectedKey(refKey(event))}><span className="timeline-marker" aria-hidden="true" /><span><strong>{event.kind}</strong><small>{event.timestamp || "Timestamp unavailable"}</small><code>{refLabel(event)}</code><em>{event.detailCode || event.claimTier}</em></span></button>)}</div>}</section>;
  ...
  return <section className="m7-view" data-testid="history-view">
    <div className="section-heading"><div><span className="eyebrow">Authoritative custody</span><h2>History</h2><p>{selection.kind} / {selection.name} · {selection.container} · {context.namespace}</p></div><span className="status-pill">{query.isFetching ? "Refreshing…" : `${projection?.truncated ? "Partial · " : ""}${projection?.totalCount ?? 0} entries`}</span></div>
    {query.isLoading ? <div className="loading-state" role="status">Loading authoritative History…</div> : query.isError ? <div className="empty-state error">History could not be read for this bound context.</div> : <>
      ...
      {!events.length ? <div className="empty-state" data-testid="history-empty">No history entries match the current authoritative projection/filter.</div> : <div className="m7-split"><div className="history-groups" data-testid="history-timeline">{renderEventList(timestampedEvents, "Timestamped events")}{renderEventList(untimestampedFacts, "Untimestamped facts")}</div>{selectedEvent ? <HistoryEventDetail event={selectedEvent} onObservation={onObservation} onProposal={onProposal} /> : <aside className="m7-detail-panel empty-detail"><span className="eyebrow">History detail</span><h3>Select an exact event or fact</h3><p>Timeline relationships are shown only from authoritative identities and provenance. Untimestamped facts are not placed on a timeline.</p></aside>}</div>}
