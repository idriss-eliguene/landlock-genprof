...
export function WorkloadDossier({ context, locator, onBack }: { context: AppContext; locator: WorkloadLocator; onBack: () => void }) {
  ...
  const [tab, setTab] = useState<"overview" | "observations" | "policy">("overview");
  ...
  const observationsQueryRaw = useInfiniteQuery({ queryKey: ["dossier-observations", ...contextQueryKey(context), selection], queryFn: ({ pageParam }) => api.observations(selection!, context, pageParam), initialPageParam: undefined as string | undefined, getNextPageParam: lastPage => lastPage.continue || undefined, enabled: Boolean(selection && ready), refetchInterval: 5_000 });
