...
<nav class="panel" aria-label="Workbench sections"><strong>Observation Workbench</strong> · Overview · Observations · Proposals · Governance · Activity · Assurance</nav><div class="panel" aria-label="Observation controls"><button id="start-observation" type="button">Start observation</button> <button id="stop-observation" type="button">Stop selected</button> <button id="generate-proposal" type="button">Generate proposal</button></div>
<section id="observation-workbench" class="panel"><h2>Observation Workbench</h2><p class="muted">Durable, workload-scoped reads. Approval, rejection, revocation, and application remain CLI-only.</p><label for="workload-picker">Workload / container</label> <select id="workload-picker"><option value="">Select a discovered container</option></select> <button id="refresh-workbench" type="button">Refresh</button><p id="workbench-message" class="notice" role="status"></p><h3>Observations</h3><ul id="observation-list"><li>Select a workload/container.</li></ul><div id="observation-detail"></div><h3>Proposals</h3><ul id="proposal-list"><li>Select a workload/container.</li></ul><p class="boundary">Proposal subject identity is Scope/Target/Container/ImageIdentity; it is not workload-UID-bound. UNKNOWN remains distinct from EMPTY and AVAILABLE.</p></section>
