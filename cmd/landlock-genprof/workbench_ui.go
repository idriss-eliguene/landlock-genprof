...
const workbenchScript = `(function () {
  ...
  app.querySelector('#start-observation').onclick = async () => { if (!selectedContext) return showError({reason:"Select a workload/container first."}); try { await post('/api/observations/start', {namespace:"{{.Namespace}}",pod:selectedContext.pod,container:selectedContext.container,sources:["filesystem"],duration:60000000000}); await load(selectedContext); } catch (x) { showError(x); } };
  app.querySelector('#stop-observation').onclick = async () => { if (!selectedObservation) return showError({reason:"Select an Observation first."}); try { await post('/api/observations/stop', {namespace:"{{.Namespace}}",observationID:selectedObservation.observationID}); await load(selectedContext); } catch (x) { showError(x); } };
  app.querySelector('#generate-proposal').onclick = async () => { if (!selectedObservation) return showError({reason:"Select a completed Observation first."}); try { await post('/api/observations/generate-proposal', {namespace:"{{.Namespace}}",observationID:selectedObservation.observationID,proposalName:"observation-" + selectedObservation.observationID}); await load(selectedContext); } catch (x) { showError(x); } };
