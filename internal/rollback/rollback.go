...
func Run(ctx context.Context, stdout io.Writer, stdin io.Reader, opts Options, name string, deps Dependencies) error {
	client, err := deps.NewDynamicClient()
	if err != nil {
	...
	if opts.ExpectedResourceVersion != "" && source.GetResourceVersion() != opts.ExpectedResourceVersion {
		return fmt.Errorf("source ApplyAttempt resourceVersion changed before rollback")
	}
	...
	currentSource, err := client.Resource(attempt.GVR).Namespace(opts.Namespace).Get(ctx, name, metav1.GetOptions{})
	if err != nil {
		return fmt.Errorf("re-reading rollback source before custody: %w", err)
	}
	if string(currentSource.GetUID()) != string(source.GetUID()) || currentSource.GetResourceVersion() != source.GetResourceVersion() {
		return fmt.Errorf("source ApplyAttempt changed before rollback custody")
	}
	...
	rbName, rbObj, err := deps.CreateRollbackAttempt(ctx, client, opts.Namespace, rbSpec)
	if err != nil {
	...
	for i := len(eligible) - 1; i >= 0; i-- {
		...
		if err := deps.ExecuteInverse(ctx, client, &sourceRecord, spec.Target); err != nil {
			...
			sourceRecord.Result = failureResult(err)
			...
			} else if applyproposal.HasSuccessfulMutation(rbStatus.Mutations) {
...
func prepareInverse(ctx context.Context, client dynamic.Interface, source attempt.MutationRecord) (attempt.MutationRecord, error) {
	...
	before := string(applyproposal.MutationSnapshot(schema.GroupVersionKind{Group: source.Group, Version: source.Version, Kind: source.Kind}, current))
	...
	return attempt.MutationRecord{
		...
		Before: before, IntendedAfter: intended, BeforeDigest: applyproposal.DigestJSON([]byte(before)),
		IntendedAfterDigest: applyproposal.DigestJSON([]byte(intended)), Result: attempt.ResultUnknown,
	}, nil
}
...
func executeInverse(ctx context.Context, client dynamic.Interface, record *attempt.MutationRecord, target k8s.GovernedTarget) error {
	...
	if current.GetKind() == "Deployment" || current.GetKind() == "StatefulSet" || current.GetKind() == "DaemonSet" {
		...
		if hasPodLockReference(current) {
			slugs = append(slugs, "podlock")
		}
		if hasSeccompReference(current) {
			slugs = append(slugs, "spo-seccompprofile")
		}
		if err := applyproposal.ValidateCompositionCompatibilityForSlugs(slugs); err != nil {
			return preDispatchFailure(err)
		}
	}
	...
	record.Result = attempt.ResultSucceeded
	record.ObservedAfter = string(applyproposal.MutationSnapshot(schema.GroupVersionKind{Group: record.Group, Version: record.Version, Kind: record.Kind}, after))
	record.ObservedAfterDigest = applyproposal.DigestJSON([]byte(record.ObservedAfter))
	return nil
}
...
func controlledMatches(record attempt.MutationRecord, current *unstructured.Unstructured) bool {
	...
	gvk := schema.GroupVersionKind{Group: record.Group, Version: record.Version, Kind: record.Kind}
	if json.Unmarshal(applyproposal.MutationSnapshot(gvk, current), &currentSnap) != nil {
		return false
	}
