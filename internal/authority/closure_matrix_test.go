...
func TestFamilyIdentitySubstitutionMatrix(t *testing.T) {
	...
	cases := []struct {
		...
		{"trust-subject", MatchRequest{Family: FamilyTrust, Attempt: a, Authority: "a", Requirement: "r", Subject: "subject", Policy: "policy", Root: "root", TypedContext: c, Scope: s, At: v.ObservedAt()}, EvaluationFactSnapshot{attempt: a, trusts: []TrustFact{trust}}, func(q *MatchRequest) { q.Subject = "other" }},
		{"verification-property", MatchRequest{Family: FamilyVerification, Attempt: a, Authority: "a", Requirement: "r", Subject: "subject", Producer: "verifier", Property: "property", TypedContext: c, Scope: s, At: v.ObservedAt()}, EvaluationFactSnapshot{attempt: a, verifications: []ResolvedVerificationFact{ver}}, func(q *MatchRequest) { q.Property = "other" }},
		...
		{"completeness-subject", MatchRequest{Family: FamilyCompleteness, Attempt: a, Authority: "a", Requirement: "r", Subject: "subject", Scope: s, At: v.ObservedAt(), RequiredCompletenessClass: comp.class}, EvaluationFactSnapshot{attempt: a, completeness: []CompletenessFact{comp}}, func(q *MatchRequest) { q.Subject = "other" }},
		{"adequacy-subject", MatchRequest{Family: FamilyAdequacy, Attempt: a, Authority: "a", Requirement: "r", Subject: "subject", Scope: s, TypedContext: c, Verifier: ad.verifier, At: v.ObservedAt(), RequiredAdequacyClass: ad.class}, EvaluationFactSnapshot{attempt: a, adequacies: []AdequacyFact{ad}}, func(q *MatchRequest) { q.Subject = "other" }},
		{"certification-property", MatchRequest{Family: FamilyCertification, Attempt: a, Authority: "a", Requirement: "r", Subject: "subject", Source: "certificate", Property: "property", Scope: s, TypedContext: c, Verifier: cert.verifier, At: v.ObservedAt()}, EvaluationFactSnapshot{attempt: a, certifications: []CertificationFact{cert}}, func(q *MatchRequest) { q.Property = "other" }},
	}
