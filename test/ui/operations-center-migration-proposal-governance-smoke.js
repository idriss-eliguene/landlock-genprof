...
async function main() {
  ...
  page.on("response", async response => { if (response.url().includes("generate-proposal") || response.url().includes("/api/governance/")) { console.error(`MUTATION_RESPONSE ${response.status()} ${response.url()} ${await response.text().catch(() => "")}`); } if (response.status() >= 400 && response.url().includes("/api/")) { if ((response.status() === 409 && response.url().includes("/api/governance/")) || (response.status() === 500 && response.url().includes("/api/governance/") && response.url().endsWith("/apply"))) expectedNegative.push(`${response.status()} ${response.url()}`); else httpErrors.push(`${response.status()} ${response.url()}`); } });
  page.on("request", request => { if (request.url().includes("generate-proposal") || request.url().includes("/api/governance/")) console.error(`MUTATION_REQUEST ${request.method()} ${request.url()} ${request.postData() || ""}`); });
  try {
    await bind(page);
