...
#
# ---------------------------------------------------------------------------
# REQUIRES A REAL-NODE CLUSTER. This scenario cannot pass on kind.
#
...
#
# BPF reports pids in the HOST kernel's pid namespace. Under kind the node is
# itself a container with its own pid namespace, so those pids do not exist
# in spod's /proc and every lookup fails — 156 failures and zero associations
# in run 32255710044.
#
# The node must therefore BE the Linux host. test/e2e/install-k3s.sh
# provisions such a cluster and asserts the topology, and
# test/e2e/preflight-spo-recorder.sh proves the recorder can resolve pids
# there before this scenario runs. Any containerized node — kind, k3d,
# minikube's docker driver — reproduces the original failure.
#
# Run by .github/workflows/spo-dmin-e2e.yml. To run by hand against a
# real-node cluster:
#
