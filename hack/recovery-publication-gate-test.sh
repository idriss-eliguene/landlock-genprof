...
fetch_helper="$root_dir/hack/recovery-fetch-tag.sh"
...
fetch_command="hack/recovery-fetch-tag.sh origin \"\$tag\" \"refs/tags/recovery-\${tag}\""
legacy_fetch="refs/tags/\${tag}:refs/tags/recovery-\${tag}"
...
grep -Fq "$fetch_command" <<<"$qualify_block"
grep -Fq "$fetch_command" <<<"$publish_block"
if grep -Fq "$legacy_fetch" <<<"$publish_block"; then
  echo "malformed duplicate-remote fetch remains in publication block" >&2
  exit 1
fi
