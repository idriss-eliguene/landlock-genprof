...
proxy_url="http://127.0.0.1:${DEMO_PROXY_PORT}"
backend_url="http://127.0.0.1:${DEMO_BACKEND_PORT}"
...
api_health_url="${proxy_url}/api/health"

# The banner is intentionally emitted only after both listeners and the
# authoritative process readiness endpoints are reachable. Metrics are
# opt-in in this disposable demo; do not print URLs for disabled listeners.
for health_url in "$startup_health_url" "$live_health_url" "$ready_health_url" "$api_health_url"; do
  health_code="$(curl -sS --max-time 5 -o /dev/null -w '%{http_code}' "$health_url" 2>/dev/null || true)"
  [ "$health_code" = 200 ] || die "Operations Center health check failed: ${health_url} HTTP ${health_code}"
done

echo "============================================================"
echo " landlock-genprof — Operations Center"
echo "============================================================"
echo ""
echo "UI"
echo ""
echo "  NEW / RECOMMENDED Operations Center (React)"
echo "  ${react_url}"
echo ""
echo "  LEGACY Operations Center"
echo "  ${legacy_url}"
echo ""
echo "Services"
echo ""
echo "  Browser / Trusted Proxy"
echo "  ${proxy_url}"
echo ""
...
echo ""
echo "  Startup"
echo "  ${startup_health_url}"
echo ""
echo "  Live"
echo "  ${live_health_url}"
echo ""
echo "  Ready"
echo "  ${ready_health_url}"
echo ""
echo "  Operations Center API health"
echo "  ${api_health_url}"
echo ""
echo "Observability"
echo ""
echo "  Operations Center metrics"
echo "  DISABLED (set LANDLOCK_GENPROF_METRICS_ENABLED=true and a metrics port to expose)"
echo ""
echo "  Executor metrics"
echo "  DISABLED (set LANDLOCK_GENPROF_METRICS_ENABLED=true and a reachable guest metrics port to expose)"
echo ""
echo "OPERATIONS_CENTER_DEMO_READY=YES"
echo "OPERATIONS_CENTER_REACT_URL=${react_url}"
echo "OPERATIONS_CENTER_LEGACY_URL=${legacy_url}"
echo "OPERATIONS_CENTER_PROXY_URL=${proxy_url}"
echo "OPERATIONS_CENTER_BACKEND_URL=${backend_url}"
echo "OPERATIONS_CENTER_STARTUP_HEALTH_URL=${startup_health_url}"
echo "OPERATIONS_CENTER_LIVE_HEALTH_URL=${live_health_url}"
echo "OPERATIONS_CENTER_READY_HEALTH_URL=${ready_health_url}"
echo "OPERATIONS_CENTER_API_HEALTH_URL=${api_health_url}"
echo "OPERATIONS_CENTER_METRICS_URL=DISABLED"
echo "EXECUTOR_METRICS_URL=DISABLED"
echo "============================================================"
echo " Recommended UI: NEW Operations Center (React)"
echo "============================================================"
