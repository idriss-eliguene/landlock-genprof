...
```bash
go install github.com/idriss-eliguene/landlock-genprof/cmd/landlock-genprof@v0.8.3

kubectl apply -f https://raw.githubusercontent.com/idriss-eliguene/landlock-genprof/v0.8.3/deploy/rbac.yaml
kubectl apply -f https://raw.githubusercontent.com/idriss-eliguene/landlock-genprof/v0.8.3/deploy/crd-securityprofileproposal.yaml
kubectl apply -f https://raw.githubusercontent.com/idriss-eliguene/landlock-genprof/v0.8.3/deploy/rbac-proposal.yaml
kubectl apply -f https://raw.githubusercontent.com/idriss-eliguene/landlock-genprof/v0.8.3/deploy/rbac-patched-manifest.yaml
```
...
of any kind). Swap `@v0.8.3` for `@latest` to track the newest tag
...
```bash
go install -ldflags "-X main.version=v0.8.3" github.com/idriss-eliguene/landlock-genprof/cmd/landlock-genprof@v0.6.1
```
...
Confirmed working as of `v0.8.3` — six real assets on the
[releases page](https://github.com/idriss-eliguene/landlock-genprof/releases)
(`v0.8.3` itself predates this pipeline being wired up; if you're
...
kubectl apply -f https://raw.githubusercontent.com/idriss-eliguene/landlock-genprof/v0.8.3/deploy/crd-traininghistory.yaml   # --history
kubectl apply -f https://raw.githubusercontent.com/idriss-eliguene/landlock-genprof/v0.8.3/deploy/rbac-history.yaml         # --history
kubectl apply -f https://raw.githubusercontent.com/idriss-eliguene/landlock-genprof/v0.8.3/deploy/rbac-restart.yaml        # --restart
```

Pinned to the `v0.8.3` tag rather than `master` on purpose — reproducible,
...
```bash
helm install landlock-genprof oci://ghcr.io/idriss-eliguene/charts/landlock-genprof --version 0.8.3
```

Confirmed published as of `v0.8.3` — check
...
(`v0.8.3` itself predates this pipeline being wired up).
...
The historical v0.8.3 installation path can optionally expose
