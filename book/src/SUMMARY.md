# Summary

[Home](index.md)

# Start Here

- [Choose your path](start-here.md)
- [Try it in a disposable cluster](docs/test-environment.md)
- [Install in an existing cluster](INSTALL.md)
- [Run the canonical demo](demo/README.md)

# Use the Product

- [The governed workflow](workflow.md)
- [Review proposals with the Workbench](workbench.md)
- [Usage guide](docs/usage.md)
  - [Generate a NetworkPolicy](docs/usage/network-policy.md)
  - [Restart the target](docs/usage/target-restart.md)
  - [Learn across multiple runs](docs/usage/multi-run-history.md)
  - [Generate a seccomp profile](docs/usage/seccomp-profile.md)
  - [Generate capabilities](docs/usage/capabilities-fragment.md)
  - [Compose a securityContext](docs/usage/composed-security-context.md)
  - [Generate a review report](docs/usage/unified-report.md)
  - [Publish a proposal](docs/usage/proposal-publishing.md)
  - [Generate a patched manifest](docs/usage/patched-manifest.md)
  - [Generate an SPO resource](docs/usage/seccompprofile-resource.md)
  - [Import SPO-derived policy](docs/usage/spo-seccomp-import.md)
- [CLI reference](cli/landlock-genprof.md)
  - [trace](cli/landlock-genprof_trace.md)
  - [synthesize](cli/landlock-genprof_synthesize.md)
  - [review](cli/landlock-genprof_review.md)
  - [approve](cli/landlock-genprof_approve.md)
  - [reject](cli/landlock-genprof_reject.md)
  - [apply-proposal](cli/landlock-genprof_apply-proposal.md)
  - [rollback](cli/landlock-genprof_rollback.md)
  - [custody-epoch](cli/landlock-genprof_custody-epoch.md)
    - [custody-epoch activate](cli/landlock-genprof_custody-epoch_activate.md)
  - [ui](cli/landlock-genprof_ui.md)
  - [doctor](cli/landlock-genprof_doctor.md)
  - [verify](cli/landlock-genprof_verify.md)
  - [explain](cli/landlock-genprof_explain.md)
  - [export](cli/landlock-genprof_export.md)
  - [diff](cli/landlock-genprof_diff.md)
  - [ABI compatibility](cli/landlock-genprof_abi.md)
    - [abi list](cli/landlock-genprof_abi_list.md)
    - [abi check](cli/landlock-genprof_abi_check.md)
  - [Evidence](cli/landlock-genprof_evidence.md)
    - [evidence show](cli/landlock-genprof_evidence_show.md)
    - [evidence list](cli/landlock-genprof_evidence_list.md)
  - [Policy](cli/landlock-genprof_policy.md)
    - [policy list](cli/landlock-genprof_policy_list.md)
    - [policy status](cli/landlock-genprof_policy_status.md)
  - [version](cli/landlock-genprof_version.md)

# Understand the System

- [Architecture overview](docs/architecture.md)
- [Security model](docs/threat-model.md)
- [Enforcement prerequisites](docs/enforcement-prerequisites.md)
- [Architecture decisions](docs/adr/README.md)

# Project

- [Demonstrated capabilities](project/progress.md)
- [Product roadmap](project/roadmap.md)
- [Versioned documentation](docs/versioned-docs.md)

# Contribute

- [Contributing](CONTRIBUTING.md)
