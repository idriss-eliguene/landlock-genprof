...
## Exact-head PR #265 qualification addendum — 2026-09-24

This addendum supersedes the earlier same-SHA qualification limitation above
for commit `1986210b73243957f322088222de7f8be09ed46e`. The remote PR head was
verified unchanged before and after the qualification. The source worktree
was detached at that exact commit and remained clean.

### Disposable environment

The existing Lima Docker context was not changed and no existing cluster was
used. A fresh k3d cluster named
`landlock-genprof-pr265-gate` was used and removed after testing.

| Component | Recorded value |
|---|---|
| Host | macOS Darwin 24.2.0, arm64 |
| VM kernel | Linux `7.0.0-31-generic`, aarch64 |
| Kubernetes | k3s `v1.31.5+k3s1` |
| kubectl | `v1.36.4` client (with the expected version-skew warning) |
| k3d | `v5.8.3` |
| Docker | client `28.4.0`, server `29.8.0` |
| SPO | `v1.0.0`, operator image digest `sha256:f0c41723c49719036d9ee21de80d721df6ba122df529e4ce87a17b3b2c202f18` |
| SPO SeccompProfile CRD | cluster-scoped, served `v1` and `v1beta1` |

The earlier kind startup failure was reproduced and diagnosed. The host file
table and Docker limits were not exhausted; the failing node had the default
per-container inotify instance limit of `128`. The disposable k3d node was
created with `nofile=524288:524288`, then its inotify limits were temporarily
raised inside that node to `1024` instances and `524288` watches. After a
single node restart, the node, CoreDNS, API readiness, and system workloads
became healthy. No host or persistent VM limit was changed.

The exact PR backend binary was built from the detached source at the required
SHA and run in production authorization mode against the disposable API
server. Its HTTP endpoints, signed trusted-proxy identity path, Kubernetes
impersonation, SSAR capability discovery, ApplyAttempt custody, and separate
profile-realizer kubeconfig were exercised. The published GHCR backend image
was not used because it was unavailable; container-image packaging is not
claimed by this qualification.

### Exact-head evidence

The successful governed apply used:

* Proposal `team-a/gate-apply-v1`, UID
  `a761bbec-591a-4965-beef-87c111d86a7a`.
* Candidate digest
  `sha256:222a2e06a9622ce4c3ab0d3ce86dde8d4a7079c98aa79cc4e46ecdb716c366c9`.
* ApplyAttempt `team-a/apply-20260924t202507-fc16aa5b92e54843`, UID
  `65b6f775-f658-4684-9560-ce888c5f30d3`, operator identity `approver-only`,
  state `APPLIED`.
* Resulting SPO profile `lg-v1-api-4b06088171cfc435`, UID
  `cd3d71f2-2146-4aa0-892b-6cc556a32432`, with `Ready=True`, reason
  `Available`, status `Installed`, and localhost profile
  `operator/lg-v1-api-4b06088171cfc435.json`.

The profile-realizer authorization was checked directly against Kubernetes:

| Identity / operation | Result |
|---|---|
| `team-a-human` get/create/update/patch/delete SeccompProfile | PASS — denied for all five verbs |
| `landlock-genprof-profile-realizer` get/create/update/patch/delete SeccompProfile | PASS — allowed for all five verbs |
| `team-a-human` impersonate service accounts | PASS — denied |
| Realizer read of the named `kube-system` Namespace | PASS |

### Exact-head negative matrix

| Scenario | Result and post-condition |
|---|---|
| Reviewer-only `reviewer-only` attempts approval | PASS — HTTP 403 `AUTHORIZATION_DENIED`; proposal remained `Reviewed`, no ApplyAttempt or SPO change |
| Cross-namespace approver `cross-namespace-approver` attempts team-A approval | PASS — HTTP 403; `gate-proposal` stayed at resourceVersion `4566`, `Approved`, actor unchanged |
| Forged `X-Environment-Namespace: team-b` and invalid session | PASS — HTTP 409 `STALE_ENVIRONMENT_CONTEXT`; no mutation |
| Stale approval precondition after a real API resourceVersion bump | PASS — HTTP 409 `CONFLICT`; proposal stayed `Approved`, no extra ApplyAttempt |
| Ownership mismatch (`target-namespace: team-b` for a team-A target) | PASS — HTTP 412 before mutation; ApplyAttempt count stayed `1`, SPO profile count stayed `1` |
| PodLock plus application-derived SPO composition | PASS — HTTP 412 stating runtime compatibility is unproven; ApplyAttempt count stayed `1` |
| Human direct cluster-scoped SPO write | PASS — Kubernetes `no` for get/create/update/patch/delete |

The signed requests were generated by the repository's `hack/authfixture`
against a disposable 32-byte HMAC secret. Credentials, tokens, kubeconfigs,
and secret material were kept outside the repository and are not part of the
evidence. Kubernetes audit sink output was not enabled; sanitized HTTP
responses, API object UIDs/resourceVersions, RBAC authorization reviews,
ApplyAttempt status, and SPO conditions are the retained evidence.

### Gate decision

The required real-cluster authorization and governed-apply scenarios passed
at the exact PR head. SPO `Ready/Installed` proves API reconciliation only;
it is not evidence of kernel-level seccomp enforcement. No product code,
master branch, release tag, or PR branch was modified. The addendum is a
working-tree documentation update in the isolated qualification worktree;
the tested source remains exactly `1986210b73243957f322088222de7f8be09ed46e`.

**FINAL_GATE: READY_FOR_OWNER_REVIEW**
