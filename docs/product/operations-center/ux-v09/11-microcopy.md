...
| "Authoritative read/action failed. The authoritative state was not changed." (shown for *every* failure, including ones with a specific known cause) | State-specific headline (`errorHeadline()`): "This Observation is not finished yet.", "Not authorized for this session.", "No usable evidence for a proposal.", etc., falling back to a message that always includes the HTTP status/code rather than nothing | The fully generic version is what a live review correctly rejected as insufficient operator UX. See [15-known-limitations.md](15-known-limitations.md). |
| Workload picker label rewritten at runtime via JS (`label[for="workload-picker"] strong` → `"Workload"`), rendered adjacent to its `<select>` with no spacing (`WorkloadDeployment/api · api`) | "Workload" authored directly in the template, stacked above the control in a `.form-field` | A real control collision found in live review; fixed structurally (label-above-control), not with a margin patch. See [15-known-limitations.md](15-known-limitations.md). |
